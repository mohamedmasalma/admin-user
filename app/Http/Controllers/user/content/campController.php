...
function all_camp(){
  ...
  function sche_camp(){
     $camp= new campaign;
    $scheduled=  $camp->all()->where('status', 'scheduled')->where('user_id',Auth::user()->id);
    return view('user.campaigns.schedualed',compact('scheduled'));
}
...
if($request->time_zone=='' ){
  ...
  if (Auth::user()->profile->paid=='no') {
    session()->flash('not_paid', 'you should subscribe to be able to create campaign');
